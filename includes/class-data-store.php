...
class Data_Store {
	 ...
	 * Ran through `woocam_data_stores`.
	 *
	 * @var array
	 */
	private $stores = array(
		'address'                => '\\Dornaweb\\WOOCAM\\Data_Stores\\Address_Data_Store',
	);
	...
	public function __construct( $object_type ) {
		...
		$this->stores      = apply_filters( 'woocam_data_stores', $this->stores );
		...
		if ( array_key_exists( $object_type, $this->stores ) ) {
			$store = apply_filters( 'woocam_' . $object_type . '_data_store', $this->stores[ $object_type ] );
			if ( is_object( $store ) ) {
				if ( ! $store instanceof \Dornaweb\WOOCAM\Interfaces\Object_Data_Store_Interface ) {
					throw new Exception( __( 'Invalid data store.', 'woocam' ) );
				}
	 ...
	 *
	 * @since 3.0.0
	 * @param WC_Data $data woocam data instance.
	 */
