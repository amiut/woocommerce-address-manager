...
abstract class Data
{
	...
	public function save() {
		 ...
		 */
		do_action( 'woocam_before_' . $this->object_type . '_object_save', $this, $this->data_store );
		 ...
		 * @param WC_Data_Store_WP $data_store THe data store persisting the data.
		 */
		do_action( 'woocam_after_' . $this->object_type . '_object_save', $this, $this->data_store );
		...
		if ( ! empty( $data ) && is_array( $data ) ) {
			...
			foreach ( $data as $meta ) {
				...
				if ( isset( $meta['key'], $meta['value'], $meta['id'] ) ) {
					$this->meta_data[] = new Woocam_Meta_Data(
						array(
							'id'    => $meta['id'],
							'key'   => $meta['key'],
							'value' => $meta['value'],
	...
	public static function generate_meta_cache_key( $id, $cache_group ) {
		return Woocam_Cache_Helper::get_cache_prefix( $cache_group ) . Woocam_Cache_Helper::get_cache_prefix( 'object_' . $id ) . 'object_meta_' . $id;
	}
	...
	public function save_meta_data() {
		...
		if ( ! empty( $this->cache_group ) ) {
			$cache_key = Woocam_Cache_Helper::get_cache_prefix( $this->cache_group ) . Woocam_Cache_Helper::get_cache_prefix( 'object_' . $this->get_id() ) . 'object_meta_' . $this->get_id();
	...
	protected function get_hook_prefix() {
		return 'woocam_' . $this->object_type . '_get_';
	}
